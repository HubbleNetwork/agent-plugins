...
**Purpose**: Register multiple Bluetooth devices with Hubble Network in a single API call. The API generates device credentials automatically.

**When to use**:
- Initial device onboarding
- Mass device provisioning (up to 1,000 devices per request)
- Fleet deployment
...
```json
{
  "n_devices": 100,           // Number of devices to provision (1-1000)
  "encryption": "AES-128-CTR" // Encryption type: AES-256-CTR, AES-128-CTR, or NONE
}
```

**Parameters**:
- `n_devices` (integer, required): Number of devices to create. Must be between 1 and 1000.
- `encryption` (string, required): Encryption type. Possible values:
  - `AES-256-CTR` - AES-256 encryption (recommended for production)
  - `AES-128-CTR` - AES-128 encryption (balanced security/performance)
  - `NONE` - No encryption (testing/development only)
...
```json
{
  "devices": [
    {
      "device_id": "ed30d987-d4cd-479e-a829-aa3c1423234f",
      "name": "",
      "created_ts": 1767129870,
      "tags": {
        "_env": "production"
      },
      "protocol": {
...
**Response Fields**:
- `device_id`: Unique UUID for the device
- `key`: Base64-encoded device encryption key (store securely!)
- `created_ts`: Unix timestamp of device creation
- `tags`: Auto-assigned tags including environment
- `protocol`: Protocol configuration

**Important Notes**:
- Device credentials (IDs and keys) are generated by the API
- Store the returned device keys securely - they cannot be retrieved later
- The API enforces daily provisioning limits per organization
- Devices are created in batches for optimal performance

**Error Responses**:
- `400 Bad Request`: Invalid n_devices value (must be 1-1000) or invalid encryption type
- `400 Bad Request`: Daily provisioned devices limit exceeded
- `401 Unauthorized`: Invalid or missing API key
- `403 Forbidden`: Insufficient permissions (requires `devices:write` scope)
...
**Endpoint**: `DELETE /api/org/{org_id}/devices`

**Purpose**: Delete multiple devices in a single request (up to 1,000 devices).

**When to use**:
- Mass device decommissioning
- Cleaning up test devices
- Fleet management and lifecycle operations
- Optimizing API usage and rate limits

**Request**:
```bash
curl -X DELETE "https://api.hubble.com/api/org/7db20441-a04c-49ba-8089-1197b3a66a4b/devices" \
  -H "Authorization: Bearer eyJhbGc..." \
  -H "Content-Type: application/json" \
  -d '{
    "device_ids": [
      "ed30d987-d4cd-479e-a829-aa3c1423234f",
      "3f8b1c2a-5d9e-4a7b-9c3d-1e5f7a8b9c0d",
...
**Parameters**:
- `device_ids` (array of UUIDs, required): Array of device IDs to delete. Must contain 1-1000 device IDs.

**Constraints**:
- Maximum 1,000 devices per request
- Device IDs must be valid UUIDs
- Deleted devices cannot be recovered
- Associated data (packets, metrics) may be retained per retention policy
...
**Response Fields**:
- `deleted`: Number of successfully deleted devices
- `failed`: Number of devices that failed to delete
- `errors`: Array of error details for failed deletions

**Error Responses**:
- `400 Bad Request`: Invalid device_ids format or empty array
- `400 Bad Request`: Too many device IDs (exceeds 1000 limit)
- `401 Unauthorized`: Invalid or missing API key
- `403 Forbidden`: Insufficient permissions (requires `devices:write` scope)
...
- Successfully deleted devices are immediately removed from all API responses
- Packet data may be retained according to your organization's retention policy
- Use with caution in production environments
- Consider implementing confirmation prompts in your application

**Best Practice**:
- Verify device IDs before deletion
- Implement audit logging for deletion operations
- Use batch operations to minimize API calls and improve performance
- Handle partial failures gracefully by checking the response
